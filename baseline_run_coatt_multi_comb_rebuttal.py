...
from baseline_model_coatt_multi_comb_rebuttal import Classifier
...
def train(epoch, data, names, window_size, batch_size, log_dict):
    ...
    if prev_best_macro < perf:
        ...
        torch.save(model.state_dict(), './model/mrda_coatt_multi_comb_4layer_batch_size_'+str(batch_size)+'_seed'+str(seed)+'.pt')
...
if __name__ == '__main__':
    ...
    (train_data, train_names), (validate_data, validate_names), (test_data, test_names) = get_data('./mrda_data_whisper_aligned/')
    ...
    for seed in seed_list:
        ...
        try:
            os.mkdir(f'./prediction_out_self_multi_comb_rebuttal_{seed}')
        except:
            pass
        ...
        try:
            ...
            for epoch in range(40):
                ...
                df.to_csv(f'./log_self_multi_comb_rebuttal_{seed}.csv')
                try:
                    os.mkdir(f'./classification_reports_self_multi_comb_rebuttal_{seed}')
                except:
                    pass

                with open(f'./classification_reports_self_multi_comb_rebuttal_{seed}/cr_{epoch}.txt', mode='w') as file:
                    file.write(cr)
